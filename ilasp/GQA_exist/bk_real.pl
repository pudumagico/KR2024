...
1 {state(TO,ID): state(TI,ID)} 1 :- unique(TO, TI).
1 {attr(TO, ATTR): is_attr(ATTR)} 1 :- common(TO, TI0, TI1).
1 { has_attr(ID, ATTR, VALUE) : is_attr_value(ID, VALUE)} 1 :- query(TO, TI, ATTR), state(TI, ID), ATTR != name, ATTR != class, ATTR != hposition, ATTR != vposition.
1 {has_attr(ID, ATTR, VALUE); has_attr(ID, ATTR, VALUE2)} 1 :- choose_attr(TO, TI, ATTR, VALUE, VALUE2), state(TI, ID).
1 {has_rel(ID2, REL, ID): has_attr(ID2, class, CLASS); has_rel(ID2, REL2, ID): has_attr(ID2, class, CLASS)} 1 :- choose_rel(TO, TI, CLASS, REL, REL2, subject), state(TI, ID).
1 {has_rel(ID, REL, ID2): has_attr(ID2, class, CLASS); has_rel(ID, REL2, ID2): has_attr(ID2, class, CLASS)} 1 :- choose_rel(TO, TI, CLASS, REL, REL2, object), state(TI, ID).

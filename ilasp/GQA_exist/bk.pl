...
% Add configuration to learn choice rules.
...
#maxv(7).   % Maximum number of variables per rule

% Modes

#modeh(bool(var(state_id), var(bool_id))).

#modeb(exist(var(state_id), var(state_id))).
#modeb(state(var(state_id), var(obj_id))).
#modeb(bool(var(state_id), var(bool_id))).
...
attr(TO, ATTR) :- common(TO, TI0, TI1), is_attr(ATTR).
has_attr(ID, ATTR, VALUE)  :- is_attr_value(ID, VALUE), query(TO, TI, ATTR), state(TI, ID), ATTR != name, ATTR != class, ATTR != hposition, ATTR != vposition.
has_attr(ID, ATTR, VALUE) :- choose_attr(TO, TI, ATTR, VALUE, VALUE2), state(TI, ID).
has_attr(ID, ATTR, VALUE2) :- choose_attr(TO, TI, ATTR, VALUE, VALUE2), state(TI, ID).
has_rel(ID2, REL, ID) :- has_attr(ID2, class, CLASS), choose_rel(TO, TI, CLASS, REL, REL2, subject), state(TI, ID).
has_rel(ID, REL, ID2) :- has_attr(ID2, class, CLASS), choose_rel(TO, TI, CLASS, REL, REL2, object), state(TI, ID).
